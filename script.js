...
(function(){
  const STORAGE_KEY='ypp-preparedness-v1';
  ...
  function updatePreparedness(){
    ...
    const state=loadPrep();
    checks.forEach((cb,i)=>{
      const key=cb.dataset.label || ('item-'+i);
      if(state[key] !== undefined) cb.checked=!!state[key];
      cb.onchange=function(){
        const current=loadPrep();
  ...
  function renderPreparedness(){
    ...
    const score=document.getElementById('preparednessScore');
    const bar=document.getElementById('preparednessBar');
    if(score) score.textContent=pct+'%';
    if(bar) bar.style.width=pct+'%';

    const next=document.getElementById('nextThreeList');
    if(next){
      next.innerHTML='';
      const missing=checks.filter(cb=>!cb.checked).slice(0,3);
      if(!missing.length){
        const el=document.createElement('div');
        el.className='mini';
        el.innerHTML='<b>Core preparedness checklist complete</b><br><span class="small">Review periodically and update when circumstances change.</span>';
        next.appendChild(el);
      } else {
        missing.forEach((cb,idx)=>{
          const el=document.createElement('div');
          el.className='mini';
          el.innerHTML='<b>'+(idx+1)+'. '+(cb.dataset.label || 'Complete this item')+'</b>';
          next.appendChild(el);
        });
      }
    }
  ...
  document.addEventListener('DOMContentLoaded', updatePreparedness);
})();
